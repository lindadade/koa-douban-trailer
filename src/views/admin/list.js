...
// const site = 'http://p5vu5doiw.bkt.clouddn.com/'
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      columns: [
      {
        ...
        dataIndex: 'poster',
        key: 'poster',
        render: (text, record) => (<img width='150' src={record.poster} />)
      },
      {
        title: '名字',
        dataIndex: 'title',
      ...
      },
      {
        ...
        render: (text, record) => (
          record.pubdate.map((it, i) => (
            <p key={i}>{moment(it.date).format('YYYY-MM-DD')} <span>{it.country}</span> <span>———————</span></p>
      ...
      },
      {
        title: '详情',
        key: '_id',
        render: (text, record) =><div style={{ textAlign: 'center', left: '800px'}}><Button> <a href={`/detail/${record._id}`} target={'_blank'}>详情</a></Button></div>
      },
      {
        title: '操作',
        key: 'action',
